...
export const assignAsset = onRequest(
  {
    cors: allowedOrigins
  },
  async (request, response) => {
      ...
      if (sheetData) {
        ...
        if (monitorRowIndex >= 0) {
          ...
          try {
            ...
            const error = `Assigning asset ${assetName} ${assetId} to ${inspector} failed due to: ${JSON.stringify(e)}`;
            logger.error(error);
            response.status(500).send({
              data: error
            });
          ...
          const error = `No asset ${assetName} ${assetId} found to assign to ${inspector}`;
          logger.error(error);
          response.status(400).send({
            data: error
          });
    ...
    try {
      ...
      if (sheetData) {
        ...
        if (monitorRowIndex >= 0) {
          ...
          const localDate = toZonedTime(new Date(), 'America/New_York');
          newMonitorRow[5] = inspector;
          newMonitorRow[16] = format(localDate, 'MM/dd/yyyy');
          const updateRowRange = `${assetName}!A${monitorRowIndex + 1}:V${monitorRowIndex + 1}`;
          try {
            await google.sheets('v4').spreadsheets.values.update({
              auth,
